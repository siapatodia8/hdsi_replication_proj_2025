...
#         # If an error happened, short-circuit with an error message.
#         if state.get("error"):
#             state["final_answer"] = (
...
#         # Pure knowledge question: no DB needed.
#         if question_type == "general_knowledge":
...
#         # SOFT FAN-IN LOGIC:
#         results = state.get("results", None)  # <-- could be None if execute hasn’t run yet
#         explanation = state.get("query_explanation")

#         # 1) If execute hasn't finished yet (results is None), don't produce anything yet.
#         if results is None:
#             return state

#         # 2) Execute finished but nothing found.
#         if not results:
#             msg = (
...
#         # 3) We have results. Make the final answer (and include explanation if present).
...
#         # If we already produced an answer earlier without the explanation and
#         # now we have it, we can enrich it. (Idempotent behavior.)
#         if explanation:
#             state["final_answer"] = f"**What the query does**: {explanation}\n\n{answer_text}"
#         else:
